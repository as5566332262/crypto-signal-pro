...
if(reason.includes("不具優勢")||reason.includes("區間中段"))return"不在支撐/壓力附近";
if(reason.includes("突破訊號仍需更多確認")||reason.includes("突破"))return"K 線確認未成立";
if(reason.includes("風險報酬"))return"RR 不足";
if(reason.includes("方向不明確"))return"方向不明，先觀望";
if(reason.includes("假突破"))return"假突破風險偏高";
if(reason.includes("趨勢不夠乾淨"))return"市場結構不乾淨";
...
const debugLite=[
{label:"市場型態",value:analysis?.marketState||"-"},
{label:"K 線確認",value:analysis?.breakoutValidation?.validated?"是":"否"},
{label:"靠近支撐",value:isNearSupport?"是":"否"},
{label:"靠近壓力",value:isNearResistance?"是":"否"},
{label:"區間過濾",value:analysis?.marketState==="ranging"?"阻擋":"通過"},
{label:"績效過濾",value:analysis?.rrOk?"通過":"阻擋"},
{label:"冷卻中",value:"否"},
{label:"本 setup 勝率",value:"-"},
{label:"本 setup 樣本",value:"-"}
];
const positionSnapshot=useMemo(()=>{const mark=analysis?.price,entry=Number.isFinite(analysis?.levels?.nearestSupport)&&Number.isFinite(analysis?.levels?.nearestResistance)?(analysis.levels.nearestSupport+analysis.levels.nearestResistance)/2:candles.at(-2)?.close??mark;
if(mark==null||entry==null)return null;
const side=analysis?.bias==="偏空"?"SHORT":"LONG",sideLabel=side==="LONG"?"多單":"空單",qtyMap={BTCUSDT:.1,ETHUSDT:2,SOLUSDT:50},qty=qtyMap[symbol]??1;
const unrealized=(side==="LONG"?mark-entry:entry-mark)*qty,value=mark*qty;
return{entry,mark,sideLabel,qty,value,unrealized};
},[analysis,candles,symbol]);
...
<div className="position-risk-stack"><div className="position-risk-item"><span className="position-label">止損：</span><span className="position-risk-value">{fmt(analysis?.stopLoss,d)}</span></div><div className="position-risk-item"><span className="position-label">止盈1：</span><span className="position-risk-value">{fmt(analysis?.takeProfit1,d)}</span></div></div>
</div>
...
<div className="card"><div style={{fontWeight:900,fontSize:22,marginBottom:12}}>Final Decision</div><div className="subgrid card-data-grid"><div className="zone"><div className="label">最終決策</div><div className="metric-value">{analysis?.finalDecision||"-"}</div></div><div className="zone"><div className="label">執行方向</div><div className="metric-value">{analysis?.setup||"-"} / {analysis?.setupType||"-"}</div></div><div className="zone"><div className="label">止損</div><div className="metric-value">{fmt(analysis?.stopLoss,d)}</div></div><div className="zone"><div className="label">止盈</div><div className="metric-value">{fmt(analysis?.takeProfit1,d)} / {fmt(analysis?.takeProfit2,d)}</div></div></div>{blockedReasons.length?<div className="note" style={{marginTop:12}}><div style={{fontWeight:800,marginBottom:6}}>本次阻擋原因</div>{blockedReasons.map((reason,idx)=><div key={idx}>• {reason}</div>)}</div>:null}<div className="note" style={{marginTop:12}}><div style={{fontWeight:700,marginBottom:6}}>Execution Checklist</div><div>• 確認支撐/壓力附近再評估。</div><div>• 至少 1 根 K 線完成確認再執行。</div><div>• RR 未達標則不進場。</div></div><div className="subgrid card-data-grid" style={{marginTop:12}}><div className="zone"><div className="label">進場區</div><div className="metric-value">{analysis?.tradePlan?.entryZone||"-"}</div></div><div className="zone"><div className="label">失效位</div><div className="metric-value">{analysis?.tradePlan?.invalidation||"-"}</div></div><div className="zone"><div className="label">目標一</div><div className="metric-value">{analysis?.tradePlan?.target1||"-"}</div></div><div className="zone"><div className="label">目標二</div><div className="metric-value">{analysis?.tradePlan?.target2||"-"}</div></div><div className="zone"><div className="label">R/R</div><div className="metric-value">{analysis?.tradePlan?.riskReward||"-"}</div></div></div></div>
<div className="card"><div style={{fontWeight:800,marginBottom:12}}>市場結構與圖表摘要</div><div className="subgrid card-data-grid"><div className="zone"><div className="label">短撐</div><div className="metric-value">{fmt(analysis?.levels?.shortSupportZone?.low,d)} ~ {fmt(analysis?.levels?.shortSupportZone?.high,d)}</div></div><div className="zone"><div className="label">短壓</div><div className="metric-value">{fmt(analysis?.levels?.shortResistanceZone?.low,d)} ~ {fmt(analysis?.levels?.shortResistanceZone?.high,d)}</div></div><div className="zone"><div className="label">結構撐</div><div className="metric-value">{fmt(analysis?.levels?.structureSupportZone?.low,d)} ~ {fmt(analysis?.levels?.structureSupportZone?.high,d)}</div></div><div className="zone"><div className="label">結構壓</div><div className="metric-value">{fmt(analysis?.levels?.structureResistanceZone?.low,d)} ~ {fmt(analysis?.levels?.structureResistanceZone?.high,d)}</div></div><div className="zone"><div className="label">市場結構</div><div className="metric-value">{analysis?.structure||"-"}</div></div><div className="zone"><div className="label">突破狀態</div><div className="metric-value">{analysis?.breakoutState||"-"}</div></div></div><div className="muted" style={{marginTop:12}}>診斷指標已降階顯示：多空分數、動能、趨勢強度與高週期細節請在側欄 Debug 查看。</div></div></div>
</div>}
